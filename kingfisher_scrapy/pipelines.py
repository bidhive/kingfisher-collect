...
def _json_loads(basename):
    return json.loads(
        pkgutil.get_data("kingfisher_scrapy", f"item_schema/{basename}.json")
    )
...
class Validate:
    def __init__(self):
        ...
        for item in ("File", "FileError", "FileItem"):
            self.validators[item] = Draft4Validator(
                _json_loads(item), resolver=resolver, format_checker=checker
            )
